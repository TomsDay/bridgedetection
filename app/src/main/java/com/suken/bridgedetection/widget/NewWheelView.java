 ...
 * Date: 7/1/14.
 */
public class NewWheelView extends ScrollView {
    public static final String TAG = NewWheelView.class.getSimpleName();
...
//    private ScrollView scrollView;
    ...
    public NewWheelView(Activity context) {
        super(context);
        context = context;
        init(context);
    }
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            NewWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    NewWheelView.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            NewWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    NewWheelView.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    NewWheelView.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
...
//            position = divided + offset + 1;
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                NewWheelView.this.smoothScrollTo(0, p * itemHeight);
            }
        });
